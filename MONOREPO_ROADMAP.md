...
- **R2: Frontend.** Same convention with a Node toolchain. The `new-service` templates gain a Node variant. The frontend consumes `contracts/openapi/*`. **Done 2026-09-25:** imported as `services/web-plusdas`, API types generated from the contract, and `new-service --kind node` (see the log). Open: the backend-ot endpoints the mocked pages need (`services/web-plusdas/docs/backend-gaps.md`: auth, health history, SLD, notes), strict TypeScript in web-plusdas, a non-root nginx image.
...
└── services/<svc>/
    ...
    ├── pyproject.toml  uv.lock  .python-version     # Python services
    │   (package.json  package-lock.json  .nvmrc)    # Node: services/web-plusdas
...
- **Port registry** (in the root CLAUDE.md and `.env.example`):
  ...
  | mock-modbus | 502 |
  | web-plusdas | 5173 (dev server 5174) |
...
- **Skills:** each one ships in the phase shown in the Skill inventory table. Only `new-service` is built in Phase 5; the rest are described here for reference.
  ...
  - `services/backend-ot/.claude/skills/add-endpoint`: router + pydantic models + integration test + `make contract`, following the existing conventions in its CLAUDE.md.
...
| 2026-09-24 | 5 | **Decisions (user):** `new-service` scaffolds a FastAPI HTTP service (same packages the repo already uses: fastapi, uvicorn, pydantic-settings; dev httpx, pytest, ruff), not a bare skeleton. Subagent models: `test-runner` = haiku, `boundary-reviewer` = inherit. |
| 2026-09-24 | 5 | `scripts/check_boundaries.py` (stdlib), `make check-boundaries`, now part of `make check` (= lint + test + check-boundaries + contracts-check). It checks: (1) cross-service imports, where a module doesn't resolve in the importing service but does in another (import roots: service dir, `src/`, `tests/`); this also catches `from app.modbus_mock_data import …` in backend-ot, where both services have a top-level `app`; (2) `../` paths that resolve into another service; (3) compose/Dockerfile/pyproject `../` paths that leave the service other than into `contracts/`; (4) mock-modbus named under any `k8s/`, `.github/` or `deploy/` outside `deploy/compose/`. Mentions by name (`make -C services/mock-modbus contract` in comments) are allowed on purpose. Verified: clean → pass; a planted import, a `../backend-ot` path, an escaping compose path and `mock-modbus` in a k8s manifest were each caught; a `../../contracts` path was allowed; all plants reverted. |
| 2026-09-24 | 5 | Root `.githooks/pre-commit` (executable bit set in the index) + `make hooks`: lint for services with staged changes, `check-boundaries`, and `contracts-check` when `services/` or `contracts/` is staged. It runs on the working tree, not only the staged content. Verified by running the hook directly: pass with the Phase 5 files staged, **blocked** (exit 2) with a planted cross-import staged. Not tested through a real `git commit` (no commits by the agent). `make hooks` not run: it is the user's once-per-clone step. Removed the empty untracked `services/backend-ot/.githooks/`. |
| 2026-09-24 | 5 | Subagents `.claude/agents/test-runner.md` (haiku; Bash/Read/Grep/Glob; reports only failures) and `boundary-reviewer.md` (inherit; `disallowedTools: Edit, Write, NotebookEdit`; Bash only for git read commands + `make check-boundaries`, by instruction, because `tools` can't restrict Bash to certain commands). |
| 2026-09-24 | 5 | `new-service` skill: `SKILL.md` + `scaffold.py` (stdlib; renders `templates/**/*.tmpl` with name/package/env-prefix/port/description; refuses an existing dir or a name compose can't use) + templates (Makefile with the standard targets + `contract`, compose.yaml, uv Dockerfile (uid 999, healthcheck on `/api/healthz`), pyproject, `.env.example`, `.dockerignore`, CLAUDE.md, `src/<pkg>/{app,config,contract}.py`, `scripts/export_openapi.py`, `tests/test_app.py` + `test_contract.py`). The skill registers the service by hand in 5 places (root Makefile `SERVICES`, `dev.yaml`, root `.env.example`, root CLAUDE.md, `contracts/README.md`). |
| 2026-09-24 | 5 | **`new-service` probe** as `probe-svc` on port 8099. Deviation: the roadmap's `_probe` is not a valid compose project name, and the scaffold now rejects it. Scaffold → `uv lock` → install → contract → registration → `lint`/`test` (2 passed) → `check-boundaries` (3 services) → `make help` lists it → staged + `make check` passed across all 3 services → `dev.yaml config -q` valid → `make up`: container healthy, `/api/healthz` → `{"ok":true}`, runs as uid 999 → `down`. A planted `from config import settings` (backend-ot's module) in probe-svc was caught. Two template bugs found and fixed: the description overflowed line length (it's now its own constant), and Starlette now deprecates `TestClient` with httpx in favour of `httpx2` (the template tests use `httpx.ASGITransport` instead, which needs no new dependency). The probe was fully removed (dir, contract, registrations, index entries, image). |
| 2026-09-24 | 5 | Root `CLAUDE.md` rewritten from the placeholder (layout, hard rules, commands, port registry, contracts, skills, agents). backend-ot CLAUDE.md gained a **Contracts** section (provides OpenAPI; consumes the mock register map for the dev seed) and a pointer to `test-runner`; the stale "pre-commit hook will… (Phase 5)" line is fixed. Other planned service CLAUDE.md edits (uv rewrite, make.ps1, ports, `Readme-dev` sibling-repo text, mock-modbus slimming) were already done in 2a/2b/3; mock-modbus is unchanged. |
| 2026-09-24 | 5 | **Verified in fresh headless sessions (`claude -p` from the root):** root skills `contracts`, `new-service`, `run-platform` and agents `boundary-reviewer`, `test-runner` are listed; after reading one file in each service, `add-endpoint`, `add-migration`, `add-mock-device` appear (directory-scoped, as intended). `test-runner` ran mock-modbus → `PASS · lint ✓ · test ✓ 183 passed`. `boundary-reviewer` reviewed the Phase 5 diff → OK (one false positive: a `__pycache__` it thought would be committed, but it is gitignored). |
| 2026-09-24 | 5 | ⚠ **Blocked, left to the user:** the auto-mode classifier denied the agent writing the root `.claude/settings.json` and `git rm` of `services/*/.claude/settings.json` (self-modification of permissions). Per Claude Code docs (checked by a subagent), nested `services/*/.claude/settings.json` are not read when Claude is launched from the root, so they are dead config either way. Also, Read-deny rules cover `cat`/`Get-Content`/Grep too, and settings changes apply to a running session. **So "reading `.env` is denied" and "routine commands don't prompt" are not verified yet.** |
| 2026-09-24 | 5 | **User applied the blocked part:** created the root `.claude/settings.json` (as proposed), `git rm` of both service `settings.json` files, `make hooks` (`core.hooksPath=.githooks` confirmed), and trusted the workspace. Staging `settings.json` was also blocked for the agent, so the user stages it. **Verified with a dummy `scripts/_envprobe/.env` (no secrets, created by the user, since the agent can't create a `.env`):** Read tool → denied; `cat` in Bash → denied; agent writing a `.env` → denied (Edit rule); `.env.example` → readable. `rm -r` of the dummy directory was **not** blocked (the Read/Edit rules don't cover deletion). Allow list, in a fresh trusted session with `--permission-mode default`: `make help` and `git status --short` ran without prompting; `curl` (not listed) was denied, as expected. **Gap:** `docker compose -f deploy/compose/dev.yaml ps` was denied, because `docker compose * ps *` needs something after `ps`. Use `make ps`, or add `Bash(docker compose * ps)` / `logs` / `config`. Two findings about the environment: headless sessions ignore project allow rules until the workspace is trusted, and the user's default permission mode is plan mode. |
| 2026-09-24 | 6 | backend-ot `README.md`: Installation / Configuration / Running Locally / Docker sections rewritten for uv + make (whole platform `make up/seed/e2e`, standalone `make -C services/backend-ot …`, host runs, tests, port overrides, image notes). The `/api/healthz` description and example response were stale (it's a no-I/O liveness check now, not a Modbus read); replaced with the live response. "Testing with a Modbus Simulator" now points at `services/mock-modbus`. `docs/RUNBOOK.md`: the two local-dev blocks (quick reference + 6.F) rewritten (no `make.ps1`, no `pae-shared-network`, no `.env` needed for containers). mock-modbus README quick start was already updated in 2b. Deploy docs untouched. |
| 2026-09-24 | 6 | **Audit of quoted commands.** Every `make …` quoted in the skills, agents, root/service CLAUDE.md, READMEs, RUNBOOK, mock docs, `contracts/README.md` and this roadmap was dry-run (`make -n`) against its Makefile: all 56 exist. The misses were English phrases or bare targets that the service CLAUDE.md files quote as run inside the service dir, all confirmed there. One stale `make openapi` in this roadmap's Phase 5 skill list was fixed. **Run live** (with host ports overridden, because the old repo's stack still holds 8000/5435/6380; not stopped without the user's OK): root `make up` → 4 containers healthy, `make seed`, `make e2e` → AGREE 3/3 devices, 155/155 points, `make ps`, `make logs`, `make restart`, `make down`; standalone `make -C services/mock-modbus up` + `make -C services/backend-ot up` → `health`, `seed-db`, `make e2e` → AGREE via `host.docker.internal`, `logs`, `ps`, `down` ×2; `make -C services/mock-modbus run` served a real Modbus read on 5020; `install` ×2, `typecheck`, `build`, `test-integration` (77 passed); lint/test/check/contract/contracts-check/check-boundaries/hooks/new-service were run in Phases 2–5. **Not run:** backend-ot `run`/`migrate` on the host (they use the user's `.env`, which the agent can't inspect, so the target DB is unknown); `format`/`lint-fix` (they rewrite files); `apply-migration`, `shell`, `clean`, `rebuild`; `cloud-*` (deploy). |
| 2026-09-24 | 6 | Left for the user to decide (outside the planned scope): `Readme-dev/DBReadme.md` and `REDIS_ACCESS.md` still use `docker-compose` and the old fixed container names (`pae-backend-ot-postgres/-redis`), and `REDIS_ACCESS.md` documents poll-cache keys the current code doesn't write. The RUNBOOK's deploy sections still offer `.\make.ps1 cloud-down/up` as an alternative (it's broken; `make cloud-down/up` works on Windows via `scripts/cloud.ps1`). |
| 2026-09-24 | 6 | **Decision (user): the repo root takes priority for running containers.** Root `make up` is a **reset**: new target `stop-all` runs `down --remove-orphans` on the dev stack, then each service's own `make -C services/<svc> down`, and only then starts the dev stack. Root `make down` = `stop-all`. A service's `up` / `up-build` / `build` / `rebuild` now depends on `require-no-dev-stack`, which fails with "the platform dev stack (pae-dev) is running; run 'make down' at the repo root first" (in backend-ot, mock-modbus and the `new-service` Makefile template). Only platform projects are touched: `pae-dev` + one per `SERVICES` entry. Other Docker projects and the `backend-ot-test-*` integration stacks are not; volumes are always kept. **Supersedes** the Phase 3 goal "dev stack and a standalone stack at once" (standalone mock + standalone backend-ot together is still supported, after a root `make down`); per-worktree `test-integration` is unaffected. Reason: day-to-day development happens only from the root. Found when a standalone `backend-ot` half-started (app stuck in `Created`) next to a running `pae-dev`. |
| 2026-09-24 | 6 | Verified live: with both standalone stacks running, root `make up` stopped them (containers removed), then `pae-dev` came up with 4 healthy containers and only `pae-dev` listed. With `pae-dev` up, mock-modbus `up`/`build` and backend-ot `up`/`up-build` each refused (exit 2, nothing created). `make seed` + `make e2e` → AGREE. A second root `make up` reset cleanly. Root `make down` → no platform projects left, `docker volume ls` unchanged. The standalone pair after that → AGREE via `host.docker.internal`. Root `make down` during a running `test-integration` stopped both standalone stacks but not `backend-ot-test-pae-platform`, and the tests passed (77). A throwaway `new-service` scaffold contained the guard on `up`/`build`, then was deleted. Docs updated: `run-platform` skill (rewritten around the rule), root CLAUDE.md, root `.env.example`, both service CLAUDE.md files, backend-ot README + RUNBOOK, `new-service` SKILL + CLAUDE.md template. |
| 2026-09-24 | — | **Bug (user): seeded devices had no STANDARDIZED points,** while devices created through `POST /api/devices/site/{id}/devices` get them (`create_device` → `generate_standardized_points`). The seeder writes ORM rows directly and never called the helper. **Decision (user): call the same helper in the seeder** (new step 3b in `tests/seed_db/seed_db.py`, persisted as `DevicePoint(**point.model_dump())` exactly like the API, skipping existing names so it stays idempotent). The alternative was to seed through the HTTP API so drift can't recur; it was rejected as a bigger rewrite, and backend-ot's CLAUDE.md now says create-path changes must be mirrored in the seeder. Regression test `tests/integration/seed_db/test_seed_db.py`: runs the real `seed()` against the test DB and compares each seeded device's STANDARDIZED points (name, address, size, data type, category, scale, unit, byte/word order) with a device of the same type created through the API, plus a reseed-without-duplicates check. It **fails without the fix** (2 failed) and passes with it. To run the seeder there, `compose.test.yaml` now mirrors the monorepo layout: the service is mounted at `/repo/services/backend-ot` and `contracts/` read-only at `/repo/contracts`, so the seeder's existing contract lookup works unchanged (allowed by `check-boundaries`). `scripts/e2e/check_backend_reads_mock.py` now asks for `category=NATIVE`, because only NATIVE points map to mock registers. Live: the reseed added the 3 points to each existing dev device (PV ×2, BESS), and `make e2e` → AGREE. |
| 2026-09-24 | — | **Added `make down-all` (user request)** at the root and in each service (+ the `new-service` template). It is DESTRUCTIVE: `down --volumes --rmi all --remove-orphans` on the dev stack, then each service's own `down-all`. **Decision (user): `--rmi all`**, i.e. built *and* pulled images (timescaledb, redis). `make down` still keeps volumes. Not touched: other Docker projects, the `backend-ot-test-*` stacks, and the external `backend-ot-uv-cache` volume. Verified live: `make down` + `make up` kept the data (2 sites before and after); `make down-all` removed the `pae-dev_*` and `backend-ot_*` volumes, the 4 images and the networks, leaving no platform projects; the next `make up` re-pulled/rebuilt, started with 0 sites, and after `make seed` gave `make e2e` → AGREE; a standalone mock-modbus stack was also removed by root `down-all`. Root CLAUDE.md now says agents only run `down-all` when the user asks. **Decision (user): no deny rule for `make down-all` for now.** The settings allow `make *`, so the only guard is the root CLAUDE.md instruction (agents run it only when the user asks). |
| 2026-09-24 | — | **Seed point order (user): per device, STANDARDIZED → NATIVE → VIRTUAL, as the API creates them** (the device POST creates the standardized points; native ones are added afterwards). The seeder had inserted every device's NATIVE points first and the STANDARDIZED ones last (ids 156–164). It now does one per-device step (standardized, flush, native, flush; VIRTUAL has no seed data, so there's only a comment marking where it would go), with a shared `point_exists` check; still idempotent. New test `test_points_are_created_in_api_order` (per device, categories non-decreasing by id, and no id interleaving across devices) **failed on the old seeder** ("mock-device-1: categories out of order by id") and passes now. backend-ot unit 130, integration 80 passed. Dev DB rebuilt with `make down-all` → `up` → `seed` (approved in the plan): device 1 std 1–3 / native 4–57, device 2 std 58–60 / native 61–86, device 3 std 87–89 / native 90–164; a reseed changed nothing; `make e2e` → AGREE. |
| 2026-09-25 | R2 | **Frontend moved into the monorepo as `services/web-plusdas`** (user request), from the separate pae-plusdas repo (plus-das `4be8e82`), **without history**: only the git-tracked files were copied (like the two existing services), leaving out the stale `bun.lockb`, the old compose/Dockerfile/Makefile/nginx.conf (replaced) and a tracked `.claude/settings.local.json`. **Why:** the UI and backend-ot change together, and in one repo the dev stack runs the whole platform with `make up` (UI on :5173 calling backend-ot same-origin). **Decision (user): `services/web-plusdas/`, not a root `web/`**, so it is a plain sibling. The root tooling needed only `SERVICES += web-plusdas`, because fan-out, stop-all/down-all and the pre-commit hook already work per `services/<svc>/` and dispatch through each service's Makefile (npm here, uv in the Python services). **The extractability invariant keeps the exit cheap:** the service depends only on `contracts/` and its own files, nothing imports from it, and nothing is hoisted (no root package.json or workspace). Its Docker build context is its own dir, so `git subtree split --prefix=services/web-plusdas` stays an afternoon's work. **Enforced by `check_boundaries.py`:** JS/TS sources and configs are scanned, and a `../` path may leave the service only into `contracts/` (for a Node service this is the import check); new check 5 rejects `scripts/`/`contracts/` reaching into any service. **Decision (user): runtime config via `/config.js` → `window.__APP_CONFIG__`**, written at container start. The build-time `VITE_RTAC_SERVER_BASE_URL` is gone, so one image runs everywhere. **Same-origin `/api`:** nginx proxies to `API_UPSTREAM` (dev stack `http://backend-ot:8000`), and the Vite dev server proxies to `localhost:8000`. No CORS, and backend-ot is unchanged. **Node 24** (active LTS; the old image used EOL Node 20), pinned by `.nvmrc` + `engines`, the npm counterpart of `.python-version`. **Decision (user): the old repo's tracked `.env.development`/`.env.production` were copied as-is** (later removed from history before pushing; see the next entry). No code reads them now; they are excluded from the Docker context and denied in `.claude/settings.json`. **Decision (user): the 10 pre-existing lint errors were fixed** in a separate commit, including a rules-of-hooks bug in SVGConnection. `make typecheck` is not yet blocking (7 pre-existing tsc errors), like backend-ot's mypy. `make test` is a placeholder. **Decision (user):** `make test-integration` checks the running container: SPA fallback, cache headers, `/config.js`, `/api` via the web origin. API types stay hand-written for now; openapi-typescript with committed output is proposed in `services/web-plusdas/docs/api-types-follow-up.md`. Verified: image built from its own dir; `make up` → 5 healthy; `test-integration` OK; `make e2e` → AGREE; `make check` green; boundary probes caught all 5 kinds of violation. |
| 2026-09-25 | R2 | **Frontend follow-ups completed (user request).** (1) Close-out: `.env.development`/`.env.production` ignored at the root. **Decision (user): they were removed from the branch history** (`git filter-branch --index-filter` over `main..HEAD`, final tree verified identical) and the branch was force-pushed over the first 5 commits already on origin, which had contained them. The user confirmed they held no secrets (only VITE_* values, public in the bundle anyway), so nothing needed rotating. `npm run *` narrowed to the named scripts. (2) **Decision (user): generated API types.** openapi-typescript (new devDependency) turns `contracts/openapi/backend-ot.openapi.json` into a committed `src/api/generated/backend-ot.ts`, imported only as `@contracts/backend-ot`. `scripts/api-types.mjs` is the one place with the contract path, so on extraction that one line changes. `make test` fails while it is stale, mirroring backend-ot's `test_contract.py`, so a backend-ot contract change turns root `make check` red until the frontend regenerates. Wire types are now aliases of generated schemas, and enum option lists are `Record<Enum, Label>`. **The contract exposed real drift, fixed:** (a) multi-point delete sent `point_ids=1,2`, which backend-ot rejected with 422 (verified live); (b) the device form's lowercase meter/relay/inverter enum lacked PV and the other 5 canonical types, so editing the seeded PV device failed validation; (c) the points grid offered 8 of 16 data types; (d) its "Reg Offset" column mapped to a field backend-ot doesn't have; (e) the stream form offered `kind=coils`, which backend-ot rejects. Client stubs for routes that don't exist were removed. (3) The 7 tsc errors were fixed (type-only), `make typecheck` is blocking, and root `make check` now runs `typecheck` (backend-ot's mypy stays non-blocking). (4) **Decision (user): vitest + jsdom + @testing-library/react** (and its required peer `@testing-library/dom`). 18 tests; reintroducing bug (a) or (b) fails 3 of them. (5) `lovable-tagger` was removed, along with the Lovable meta tags and the package name. (6) The `new-service` skill gained `--kind node` (templates in `templates/node/`, Python ones moved to `templates/python/` with identical output). A throwaway scaffold passed lint, strict tsc, 11 tests, an image build and `test-integration`, and was then deleted. web-plusdas gained the `add-api-call` skill. (7) **Decision (user): mocked pages stay mocked.** backend-ot has no auth, health history, SLD or notes API. `services/web-plusdas/docs/backend-gaps.md` lists what each needs, plus contract-accuracy gaps found on the way (defaulted request fields published as required; response enums typed as plain `str`; SSE payloads undescribed). |
